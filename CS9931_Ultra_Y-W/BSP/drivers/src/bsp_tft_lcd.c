...
/* 移植重定向 */
#include <rtthread.h>
// #define bsp_DelayMS(t)		rt_thread_delay(t/10)
...
{
    RA8875_PutPixel(x,y,*(u16 *)pixel);
}

/* 获取像素点颜色 */
void rt_hw_lcd_get_pixel(char* pixel, int x, int y)
{
    *(rt_uint16_t*)pixel = RA8875_GetPixel(x,y);
}
...
{
	if(x1<x2){
		RA8875_DrawLine(x1,y,x2,y,*(u16 *)pixel);
	}
	else {
		RA8875_DrawLine(x2,y,x1,y,*(u16 *)pixel);
	}
}
...
{
	if(y1<y2){
		RA8875_DrawLine(x,y1,x,y2,*(u16 *)pixel);
	}
	else {
		RA8875_DrawLine(x,y2,x,y1,*(u16 *)pixel);
	}
}
...
{
	RA8875_DrawBMP(x,y,1,size,(uint16_t *)pixels);
}
...
// 	{
// 		u16 i,j;
// 		char *p=(char *)play_hdh+20;
// 		for(i=0;i<0x16;i++)
// 		for(j=0;j<0x1c;j++)
// 		{
// 			rt_hw_lcd_set_pixel(p,100+j,100+i);
// 			p+=2;
// 		}
// 	}
}
