...
SD_Error SD_WaitReadOperation(void)
{
  ...
  timeout = 0x10000;//SD_DATATIMEOUT;//2016.9.2 wangxin
    ...
    while(retry)
    {
            ...
            ((rt_uint32_t)buffer > 0x20080000))
        {
            ...
            for (index = 0; index < nr; index ++)
            {
				...
				(part.offset + index) * SECTOR_SIZE + pos*SECTOR_SIZE, SECTOR_SIZE);
...
static rt_size_t rt_sdcard_write (rt_device_t dev, rt_off_t pos, const void* buffer, rt_size_t size)
{
	...
	else
	{
        if (nr == 1)
        {
            status = SD_WriteBlock((rt_uint8_t*)buffer,
                part.offset * SECTOR_SIZE + pos*SECTOR_SIZE, SECTOR_SIZE);
        }
        else
        {
            status = SD_WriteMultiBlocks((rt_uint8_t*)buffer,
                part.offset * SECTOR_SIZE + pos*SECTOR_SIZE, SECTOR_SIZE, nr);
        }
	}
