...
const rtgui_color_t selected_color = RTGUI_RGB(85, 126, 213);//RTGUI_RGB(0xc0, 0xc0, 0xc0);
...
#include "logo.hdh"
struct rtgui_image_hdcmm logo_image = RTGUI_IMAGE_HDC_DEF(2, 0x10, 0x10, logo_hdh);
...
#undef RGB_FACTOR
		...
		//自己添加的 logo
		{
			rtgui_rect_t image_rect={0,0,16,16};
			rect.x1 -= 2;
			rtgui_rect_moveto_align(&rect, &image_rect, RTGUI_ALIGN_LEFT | RTGUI_ALIGN_CENTER_VERTICAL);
			rtgui_image_blit((rtgui_image_t *)&logo_image, dc, &image_rect);
			rect.x1 += 20;
		}
